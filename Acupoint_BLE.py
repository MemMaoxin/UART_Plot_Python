...
def serial_xx():
    ...
    while True:
        ...
        if count:
            ...
            k = 0
            while k + 34 < data_len:
                if data_bytes[k] == 0XFF and data_bytes[k + 1] == 0X88:
                    t = time.time()
                    rate = rate + 1
                    if control:
                        f.write('\r\n' + str(round(t * 1000)) + ' ')
                    ...
                    for k2 in range(4):
                        for k3 in range(4):
                            data_put = data_bytes[k + 2 + k2 * 8 + k3 * 2] * 256 + data_bytes[k + 3 + k2 * 8 + k3 * 2]
                            if data_put > 5000:
                                que[k3].put(0)
                            else:
                                que[k3].put(data_put)
...
class MainWidget(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        for k, graph_title in zip(range(4),
                                  ['皮电1', '皮电2', '皮电3', '皮电4']):
        ...
        for k, p, d, color in zip(range(4), pw, data, ['r', 'g', 'y', 'b']):
