...
def serial_xx():
    ...
    while True:
        ...
        if count:
            ...
            k = 0
            while k + 21 < data_len:
                if data_bytes[k] == 0XFF and ((data_bytes[k + 19] == 0XAA and data_bytes[k + 20] ==0XAA) or (data_bytes[k + 19] == 0XDD and data_bytes[k + 20] ==0XDD)):
                    #p = data_bytes[k + 1] - 0X30
                    t = time.time()
                    rate = rate + 1
                    if control:
                        f.write('\r\n' + str(round(t * 1000)) + ' ')
                    #if data_bytes[k + 2] != 0XFF and data_bytes[k + 2] != 0XF0:
                    for k2 in range(3):
                        for k3 in range(3):
...
class MainWidget(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        for k, graph_title in zip(range(5),
                                  ['R2两端电压', 'RT1两端电压', 'RT2两端电压', 'RT1阻值', 'RT2阻值']):
        ...
        for k, p, d, color in zip(range(5), pw, data, ['r', 'g', 'y', 'b', 'm']):
            curve.insert(k, (p.plot(d, pen=color)))
            #p.setRange(yRange=[1000, 5000])
        ...
        main_layout.addWidget(self.pushButton, 6, 1, 1, 2)
...
def rate_refresh():
    ...
    for k in range(5):
        ...
        velocity = rate
        if SD_open == 0:
            label[k].setText(' Efficiency:  %d %%' % valid + ' SD STOP')
        else:
            label[k].setText(' Efficiency:  %d %%' % valid + ' SD OPEN!!')
